...
with(beaver_data,{
  qqnorm(temp[new_activity == "yes"],
         ...
         ,qqline(temp[new_activity == "yes"]))
})

with(beaver_data,{
  qqnorm(temp[new_activity == "no"],
         ,main = "Beavers inactive data"
         ,qqline(temp[new_activity == "no"]))
})



normality_test <- shapiro.test(beaver_data$temp)
normality_test$p.value

tapply(temp,new_activity,shapiro.test)

#7.763623e-05 implies that the temp is not normally distributed
#activity is normally distributed

wilcox.test(dependent ~ independent)
wilcox.test(temp ~ new_activity)

# p-value  2.2e-16 < 0.05 , which means that the null hypothesis is rejected 
# and it can be concluded as body temperature is affected by activity



#define h0 , h1
#find dependent and independent variables
#detemine the normality_test
#perform test
